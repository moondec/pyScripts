...
MANUAL_TIME_SHIFTS = {
    ...
    'TU_MET2min': 'STACJA_TU_COMMON', 'TU_MET30S': 'STACJA_TU_COMMON', 'TU_MET5S': 'STACJA_TU_COMMON', 'TU_MET1S': 'STACJA_TU_COMMON',
    ...
    'ME_DOWN_MET30min' : 'ME_DOWN_STATION_COMBINED', 'ME_DOWN_MET1min' : 'ME_DOWN_STATION_COMBINED',
...
# --- Funkcje pomocnicze ---
def load_cache(cache_path): # ... bez zmian
    if os.path.exists(cache_path):
        try:
            with open(cache_path, 'r', encoding='utf-8') as f: return json.load(f)
        except (json.JSONDecodeError, IOError): pass
    return {}
def save_cache(data, cache_path): # ... bez zmian
    try:
...
def log_chronology_error(file_path, problem_indices): # ... bez zmian
    try:
...
def decode_csi_fs2_float(raw_short_int_le: int) -> float: # ... bez zmian
    ...
    return rtn
def get_tob1_metadata(file_path): # ... bez zmian
    try:
...
def read_tob1_data(file_path,metadata): # ... bez zmian
...
def get_toa5_metadata(file_path): # ... bez zmian
    try:
...
def read_toa5_data(file_path,metadata): # ... bez zmian
...
def read_simple_csv_data(file_path): # ... bez zmian
    try:
...
def apply_timezone_correction(df, file_id, timestamp_col='TIMESTAMP'):
    """
    Stosuje korekty stref czasowych w sposób odporny na błędy, bazując na ujednoliceniu do UTC.
    """
    if df.empty or timestamp_col not in df.columns or df[timestamp_col].isnull().all():
        return df

    # Krok 1: Pracuj na kopii i upewnij się, że kolumna czasu jest typu datetime
    df_out = df.copy()
    ...
    # Krok 2: Ujednolić WSZYSTKIE znaczniki czasu do spójnego formatu UTC.
    # To jest kluczowy krok zapobiegający błędom typów.
    if df_out[timestamp_col].dt.tz is None:
        # Jeśli dane są naiwne, zakładamy, że są w UTC i je lokalizujemy
        df_out.loc[:, timestamp_col] = df_out[timestamp_col].dt.tz_localize('UTC', ambiguous='NaT', nonexistent='NaT')
    else:
        # Jeśli dane mają już strefę, konwertujemy je do UTC
        df_out.loc[:, timestamp_col] = df_out[timestamp_col].dt.tz_convert('UTC')
    
    # Ponownie usuń wiersze, które mogły stać się nieprawidłowe podczas lokalizacji
    df_out.dropna(subset=[timestamp_col], inplace=True)
    if df_out.empty:
        return df_out
    
    # Krok 3: Znajdź konfigurację dla pliku
    config_entry = TIMEZONE_CORRECTIONS.get(file_id)
    final_config = None
    if isinstance(config_entry, str):
        alias_name = config_entry
        if alias_name in TIMEZONE_CORRECTIONS and isinstance(TIMEZONE_CORRECTIONS[alias_name], dict):
            final_config = TIMEZONE_CORRECTIONS[alias_name]
    elif isinstance(config_entry, dict):
        final_config = config_entry

    # Domyślna strefa docelowa
    target_tz = final_config['target_tz'] if final_config and 'target_tz' in final_config else 'Etc/GMT-1'
    
    # Krok 4: Zastosuj specyficzne reguły korekty, jeśli istnieją
    if final_config:
        ...
        # Stwórz maskę na podstawie spójnych danych UTC
        pre_fix_mask = df_out[timestamp_col] <= correction_end_date
        
        if pre_fix_mask.any():
            # "Taniec stref czasowych" jest potrzebny, aby poprawnie zinterpretować czas lokalny zapisany jako UTC
            ...
            # Bezpiecznie przypisz skonwertowane wartości
            df_out.loc[pre_fix_mask, timestamp_col] = pre_fix_series.dt.tz_convert('UTC')

    # Krok 5: Ostateczna konwersja całej, spójnej kolumny UTC do docelowej strefy czasowej
    df_out.loc[:, timestamp_col] = df_out[timestamp_col].dt.tz_convert(target_tz)
    
    return df_out.dropna(subset=[timestamp_col])
def apply_manual_time_shifts(df, file_id, timestamp_col='TIMESTAMP'): # ... bez zmian
    ...
    return df_out
def apply_calibration(df, file_id): # ... bez zmian
    ...
    for col_name, rules_list in column_rules.items():
        ...
        for i, rule in enumerate(rules_list):
            try:
                ...
                multiplier = float(rule['multiplier']); addend = float(rule['addend'])
...
def align_timestamp(df, timestamp_col='TIMESTAMP', force_interval=None):
    ...
    if not force_interval: # Jeśli nie ma wymuszonego interwału, zwróć bez zaokrąglania
        return df_processed, "no_interval_forced"
...
def main(input_dirs_arg, output_dir_arg, file_id_filter_arg, overwrite_arg):
    ...
    if overwrite_arg: print("Info: Użyto --overwrite. Pliki wynikowe zostaną nadpisane.")

    # --- Ustalanie konfiguracji dla bieżącego uruchomienia ---
    if not file_id_filter_arg:
        print("BŁĄD: Podanie argumentu --file_id jest wymagane w tej wersji skryptu.")
        return
    ...
    if not source_ids_to_process or not effective_force_interval:
        print(f"BŁĄD: Definicja dla grupy '{file_id_filter_arg}' jest niekompletna. Wymagane są 'source_ids' i 'interval'.")
        return
        
    print(f"\nPrzetwarzanie dla grupy: '{source_id_for_filename}'")
    print(f"Docelowy interwał: '{effective_force_interval}'")
    
    # --- NOWA ARCHITEKTURA: Wczytaj wszystko, posortuj, przetwórz raz ---
    # Krok 1: Znajdź i posortuj wszystkie pliki
    ...
    # Krok 2: Wczytaj wszystkie pliki do jednej, dużej ramki danych
    print("\nKrok 2: Wczytywanie i łączenie danych w jedną całość...")
    list_of_dfs = []
    ...
    # d) Korekty czasu (strefy, manualne przesunięcia)
    ...
    # e) Kalibracja wartości
    master_df = apply_calibration(master_df, file_id_filter_arg)
    
    # f) Wyrównanie czasu (zaokrąglenie)
    master_df, _ = align_timestamp(master_df, force_interval=effective_force_interval)
    
    # g) Ostateczna deduplikacja na podstawie czasu (inteligentne scalanie)
    ...
    if final_df.empty: print("Zakończono. Brak danych po finalnym przetworzeniu."); return
            
    df_to_save = final_df.reset_index().sort_values(by='TIMESTAMP')

    # Krok 4: Podział na lata i zapis
    print(f"\nKrok 4: Zapisywanie wyników z interwałem '{effective_force_interval}'...")
