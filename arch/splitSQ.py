...
CACHE_FILE_PATH = BASE_DIR / ".splitdata_cache.json"
LOG_FILE_PATH = BASE_DIR / "split_data.log"
...
def save_dataframe_to_sqlite(df: pd.DataFrame, config: dict, lock: multiprocessing.Lock):
    """
    (Wersja 6.4) Zawiera poprawkę na błąd 'type Timestamp is not supported'
    przez konwersję dat na tekst przed wykonaniem zapytania SQL.
    """
    if df.empty:
        return
    ...
    tz_config_key = TIMEZONE_CORRECTIONS.get(group_id)
    if tz_config_key:
        tz_config = TIMEZONE_CORRECTIONS.get(tz_config_key) if isinstance(tz_config_key, str) else tz_config_key
        if tz_config: target_tz = tz_config.get('target_tz', 'Etc/GMT-1')

    # Dodajemy metadane do ramki danych
    df['group_id'] = group_id
    df['interval'] = config.get('interval', 'N/A')
    df['latitude'] = coords['lat']
    df['longitude'] = coords['lon']
    
    # Ustawiamy indeks, aby łatwo pobrać klucze główne
    ...
    with lock:
        try:
            ...
            # === NOWA LOGIKA: UŻYCIE TABELI TYMCZASOWEJ ===
            
            # Krok 1: Zapisz nowe dane do tabeli tymczasowej. Zostanie ona usunięta po zamknięciu połączenia.
            df.reset_index().to_sql('temp_upsert_data', conn, if_exists='replace', index=False)

            # Krok 2: Użyj JOIN, aby wydajnie odczytać pasujące, istniejące wiersze z bazy.
            query = """
                SELECT t1.* FROM data AS t1
                INNER JOIN temp_upsert_data AS t2 
                ON t1.group_id = t2.group_id AND t1.TIMESTAMP = t2.TIMESTAMP
            """
            existing_df = pd.read_sql_query(query, conn, parse_dates=['TIMESTAMP'])
            
            # === KONIEC NOWEJ LOGIKI ===
                
            if not existing_df.empty:
                # Po wczytaniu dane są naiwne, lokalizujemy je do tej samej strefy, w której są nowe dane
                existing_df['TIMESTAMP'] = existing_df['TIMESTAMP'].dt.tz_localize(target_tz, nonexistent='NaT', ambiguous='NaT').dropna()
            ...
            # Zapisujemy czas jako naiwny (tekst bez informacji o strefie)
    ...
    for year, new_raw_dfs_list in sorted(data_by_year.items()):
        try:
            ...
            if not matlab_df.empty:
                ...
                new_data_for_year = pd.merge(new_data_for_year, matlab_df, on='TIMESTAMP', how='left', suffixes=('', '_mat'))
...
def main():
    """
    Główna funkcja orkiestrująca w architekturze hybrydowej dla zapisu do SQLite.
    """
    ...
    # === POPRAWKA: Dodanie brakującego argumentu --db-path ===
    ...
    # === POPRAWKA: Dodanie ścieżki do bazy do konfiguracji ===
    group_config['db_path'] = args.db_path
