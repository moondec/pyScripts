...
set "PYTHON_SCRIPT_PATH=C:\Users\marek.urbaniak\Documents\pyScripts\arch\splitkopia.py"
...
REM if not exist "%INPUT_PATH%\" ( echo Ostrzezenie: Katalog dla stacji TLEN1 nie istnieje. Pomijam. ) else (
    REM for %%F in (%TLEN1_FIDS%) do (
        REM echo. & echo --- Przetwarzam grupe: %%F ---
        REM python "%PYTHON_SCRIPT_PATH%" --db-path "%DB_PATH%" -i "%INPUT_PATH%" -o "%OUTPUT_PATH%" -fid "%%F" --jobs 8 --log-level DEBUG --no-cache
    REM )
REM )
...
REM if defined TOP_DIRS_LIST (
    REM for %%F in (%MEZYK_TOP_FIDS%) do (
        ...
        REM python "%PYTHON_SCRIPT_PATH%" -i %TOP_DIRS_LIST% -o "%BASE_OUTPUT_PATH%\ME" -fid "%%F" 
    REM )
REM ) else (
...
if defined DOWN_DIRS_LIST (
    for %%F in (%MEZYK_DOWN_FIDS%) do (
        ...
        python "%PYTHON_SCRIPT_PATH%" -i %DOWN_DIRS_LIST% -o "%BASE_OUTPUT_PATH%\ME" -fid "%%F" 
    )
) else (
