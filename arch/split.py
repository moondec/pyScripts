...
# def load_matlab_data(year: int, config: dict) -> pd.DataFrame:
    # """
    # (Wersja 3.1) Wczytuje dane z plików .mat. Weryfikuje interwał czasowy
    # bezpośrednio z danych w plikach 'tv.mat', a nie z nazwy folderu,
    # i przetwarza wszystkie pasujące katalogi.
    # """
    ...
    # try:
        # station_code = group_id[:2].upper()
        # station_code = station_code.replace('TL', 'TR').replace('TL2', 'TR2')
        # base_project_path = Path(main_input_path_str).parent.parent
        # matlab_base_path = base_project_path / f"met-data_{station_code}"
    # except Exception:
        # return pd.DataFrame()

    # year_path = matlab_base_path / str(year)
    # if not year_path.exists():
        # return pd.DataFrame()

    # potential_paths = list(year_path.glob('*/zero_level'))
    # if not potential_paths:
        # return pd.DataFrame()

    # target_interval_td = pd.to_timedelta(config.get('interval', 'NaT'))
    # if pd.isna(target_interval_td):
        # logging.warning(f"Brak zdefiniowanego interwału dla grupy '{group_id}'. Nie można zweryfikować danych z MATLABa.")
        # return pd.DataFrame()

    # all_valid_matlab_dfs = []

    # for data_path in potential_paths:
        # try:
            # # --- NOWA, INTELIGENTNA WERYFIKACJA INTERWAŁU ---
            
            # # 1. Znajdź pierwszy z brzegu plik z wektorem czasu
            # time_files = list(data_path.glob('tv*.mat'))
            # if not time_files:
                # logging.debug(f"Pominięto katalog {data_path}, brak pliku tv*.mat.")
                # continue

            # # 2. Wczytaj i przeanalizuj wektor czasu, aby ustalić RZECZYWISTY interwał
            # time_vector_raw = loadmat(time_files[0])['tv'].flatten()
            # timestamps = pd.Series([matlab_to_datetime(t) for t in time_vector_raw])
            # median_interval = timestamps.diff().median()

            # # 3. Porównaj z interwałem docelowym z małą tolerancją
            # if pd.isna(median_interval) or abs(median_interval - target_interval_td) > pd.Timedelta('1s'):
                # logging.debug(f"Pominięto katalog {data_path}. Wykryty interwał ({median_interval}) nie pasuje do docelowego ({target_interval_td}).")
                # continue # Przejdź do następnego potencjalnego katalogu
            
            # # --- KONIEC WERYFIKACJI ---

            # # Jeśli interwał pasuje, wczytaj wszystkie dane z tego katalogu
            # logging.info(f"Interwał zgodny. Wczytywanie danych z {data_path}")
            
            # is_monthly = 's' in config.get('interval', '') and int(re.sub(r'\D', '', config.get('interval', ''))) < 5
            ...
            # for month in months_to_check:
                ...
                # # Wczytujemy czas ponownie, aby mieć pewność, że pasuje do zmiennych
                ...
                # for var_file in var_files:
                    ...
                    # # Upewnij się, że klucz do danych jest poprawny
                ...
                # all_valid_matlab_dfs.append(pd.DataFrame(matlab_data))

        # except Exception as e:
            # logging.warning(f"Błąd podczas weryfikacji lub wczytywania danych z {data_path}: {e}")
            # continue

    # if not all_valid_matlab_dfs:
        # return pd.DataFrame()

    # final_df = pd.concat(all_valid_matlab_dfs, ignore_index=True)
...
def apply_value_range_flags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_out = df.copy()
    # === BLOK DIAGNOSTYCZNY NR 1: Sprawdzenie duplikatów na wejściu ===
    cols = pd.Series(df_out.columns)
    if cols.duplicated().any():
        duplicated_cols = cols[cols.duplicated()].unique()
        logging.error(f"DIAGNOZA: Wykryto zduplikowane kolumny na wejściu do 'apply_value_range_flags': {list(duplicated_cols)}")
    # === KONIEC BLOKU DIAGNOSTYCZNEGO NR 1 ===
...
def apply_all_corrections(df: pd.DataFrame, config: dict, context_name: str) -> pd.DataFrame:
    ...
    # df = apply_column_mapping(df, config) # Przeniesione do process_and_save_yearly_data()
...
def clean_column_names(df: pd.DataFrame) -> pd.DataFrame:
    """
    Czyści nazwy kolumn w DataFrame, usuwając sufiksy dodane przez
    programy do synchronizacji plików (np. '.sync-conflict-...').
    """
    # Wyrażenie regularne, które znajduje kropkę, "sync-conflict", i wszystko po tym
    conflict_pattern = re.compile(r'\.sync-conflict-.*$')
    
    # Tworzymy słownik mapujący: {'stara_nazwa': 'nowa_nazwa'}
    # Zastosujemy to tylko do kolumn, które faktycznie pasują do wzorca
    rename_map = {
        col: conflict_pattern.sub('', col)
        for col in df.columns if isinstance(col, str) and conflict_pattern.search(col)
    }

    if rename_map:
        df.rename(columns=rename_map, inplace=True)
        # logging.debug(f"Oczyszczono nazwy kolumn z konfliktów synchronizacji: {list(rename_map.keys())}")
        
    return df
    
def scan_for_files(input_dirs: List[str], source_ids: List[str]) -> List[Path]:
    """
    (Wersja 2.0) Skanuje katalogi w poszukiwaniu pasujących plików,
    ignorując pliki z konfliktami synchronizacji.
    """
    all_file_paths = []
    for input_dir in input_dirs:
        ...
        for p_file in p_input.rglob('*'):
            # === POCZĄTEK POPRAWKI ===
            # Jeśli nazwa pliku zawiera "sync-conflict", natychmiast go pomiń
            if "sync-conflict" in p_file.name:
                # logging.debug(f"Pominięto plik z konfliktem synchronizacji: {p_file.name}")
                continue
            # === KONIEC POPRAWKI ===
            
            if p_file.is_file():
                file_stem = p_file.stem
                for sid in source_ids:
                    if (sid.endswith('$') and file_stem.endswith(sid.rstrip('$'))) or (sid in p_file.name):
...
# def save_results_to_csv(df_to_save: pd.DataFrame, config: dict, lock: multiprocessing.Lock):
    # """
    # (WERSJA OSTATECZNA dla CSV) Zapisuje dane do rocznych plików CSV, implementując
    # logikę "wypełniania braków" (Read-Merge-Write) i zapewniając zachowanie
    # wszystkich kolumn, w tym flag jakości.
    # """
    # if df_to_save.empty:
        # return
    ...
    # # Grupujemy dane do zapisu po latach, na wypadek gdyby jeden plik/partia obejmowały przełom roku
    # for year, new_data_for_year in df_to_save.groupby(df_to_save['TIMESTAMP'].dt.year):
        # output_filepath = output_dir / str(year) / f"{group_id}.csv"
        
        # # Sekcja krytyczna chroniona blokadą, aby uniknąć konfliktów
        # with lock:
            # output_filepath.parent.mkdir(parents=True, exist_ok=True)
            
            # try:
                # # Wczytaj istniejący plik (dane są w nim "naiwne")
                # existing_df = pd.read_csv(output_filepath, parse_dates=['TIMESTAMP'], low_memory=False)
                # # Połącz stare i nowe dane. Pandas sam uzupełni brakujące kolumny jako NaN.
                # all_data = pd.concat([existing_df, new_data_for_year], ignore_index=True)
                # logging.debug(f"Połączono nowe dane z istniejącym plikiem dla roku {year}.")
            # except FileNotFoundError:
                # all_data = new_data_for_year # Plik nie istnieje, używamy tylko nowych danych
            
            # # Scalanie, które inteligentnie wypełnia luki w danych i flagach
            # # Użycie `first()` weźmie pierwszą niepustą wartość dla każdej kolumny w każdej grupie czasowej.
            # merged_df = all_data.groupby('TIMESTAMP').first().reset_index()

            # # Porządkowanie kolumn, aby TIMESTAMP był pierwszy
            # all_columns = list(merged_df.columns)
            # if 'TIMESTAMP' in all_columns:
                # all_columns.insert(0, all_columns.pop(all_columns.index('TIMESTAMP')))
                # merged_df = merged_df[all_columns]

            # # Sortowanie chronologiczne jako ostatni krok przed zapisem
            # merged_df.sort_values(by='TIMESTAMP', inplace=True)
            
            # logging.info(f"Zapisywanie {len(merged_df)} wierszy do pliku: {output_filepath}")
            # # Zapis do CSV - format daty bez strefy czasowej, zgodnie z Twoją decyzją
            # merged_df.to_csv(output_filepath, index=False, date_format='%Y-%m-%d %H:%M:%S')

def save_final_dataframe_to_csv(final_df: pd.DataFrame, year: int, config: dict, lock: multiprocessing.Lock):
    """
    (Wersja ostateczna) Prosta funkcja, której jedynym zadaniem jest zapisanie
    finalnej, w pełni przetworzonej ramki danych dla danego roku do pliku CSV.
    """
    group_id = config['file_id']
    output_dir = Path(config['output_dir'])
    output_filepath = output_dir / str(year) / f"{group_id}.csv"
    ...
    with lock:
        ...
        logging.info(f"Zapisywanie {len(final_df)} wierszy do pliku CSV: {output_filepath}")
        final_df.to_csv(output_filepath, index=False, date_format='%Y-%m-%d %H:%M:%S')
...
def process_binary_file(args: tuple) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        df = filter_by_realistic_date_range(df, file_path) # Ten filtr operuje na surowym czasie
...
def worker_task_binary(args: tuple):
    """
    Zadanie dla procesu roboczego: przetwarza JEDEN plik binarny i zapisuje wynik.
    """
    file_path, config, lock = args
    try:
        # Używamy tutaj funkcji, która przetwarza tylko pliki binarne
        processed_df = process_binary_file((file_path, config))
        
        if processed_df is not None and not processed_df.empty:
            save_results_to_csv(processed_df, config, lock)
            return True
    except Exception as e:
        logging.error(f"Nieodwracalny błąd w procesie roboczym dla pliku {file_path.name}: {e}", exc_info=True)
    return False
def process_and_save_yearly_data(raw_dfs: List[pd.DataFrame], config: dict, lock: multiprocessing.Lock):
    """
    (Wersja ostateczna 2.0) Zawiera mechanizm scalania zduplikowanych kolumn,
    który pojawiają się po połączeniu danych z wielu źródeł.
    """
    if not raw_dfs:
    ...
    # Przetwarzamy dane rok po roku, aby oszczędzać pamięć
    for year, new_raw_dfs_list in sorted(data_by_year.items()):
        try:
            logging.info(f"--- Przetwarzanie roku: {year} | Grupa: {config['file_id']} ---")
            
            # Krok 1: Połącz wszystkie nowe, surowe dane dla danego roku
            new_data_for_year = pd.concat(
                [df.reset_index(drop=True) for df in new_raw_dfs_list], 
                ignore_index=True
            )
            
            # Krok 2: Wzbogać o dane z MATLABa
            matlab_df = load_matlab_data(year, config)
            if not matlab_df.empty:
                matlab_df['TIMESTAMP'] = pd.to_datetime(matlab_df['TIMESTAMP'])
                
                # Ustawiamy TIMESTAMP jako indeks w obu ramkach, aby je poprawnie dopasować
                new_data_for_year.set_index('TIMESTAMP', inplace=True)
                matlab_df.set_index('TIMESTAMP', inplace=True)
                
                # Uzupełnij braki w `new_data_for_year` danymi z `matlab_df`.
                # Dane, które już istnieją w `new_data_for_year`, pozostaną nietknięte.
                merged_data = new_data_for_year.combine_first(matlab_df)
                
                # Przywróć TIMESTAMP jako kolumnę
                new_data_for_year = merged_data.reset_index()

            # Krok 3: Scal wiersze o tym samym czasie (np. z różnych czujników) w jedną całość
            merged_df = new_data_for_year.groupby('TIMESTAMP').first().reset_index()

            # Krok A: Zastosuj mapowanie nazw jako pierwszy krok. Może to stworzyć duplikaty.
            mapped_df = apply_column_mapping(merged_df, config)

            # Krok B: Sprawdź i scal ewentualne duplikaty kolumn powstałe po mapowaniu.
            # === POCZĄTEK KLUCZOWEJ POPRAWKI ===
            # Sprawdzanie i scalanie zduplikowanych NAZW KOLUMN
            cols = pd.Series(merged_df.columns)
            if cols.duplicated().any():
                logging.warning(f"Wykryto zduplikowane kolumny w danych dla roku {year}. Scalanie...")
                
                # NOWA, ZALECANA METODA: Transpozycja -> Grupowanie po indeksie -> Transpozycja
                merged_df = merged_df.T.groupby(level=0).first().T
                
                # KRYTYCZNY KROK: Przywracanie typów danych po transpozycji
                logging.debug("Przywracanie typów danych po scaleniu kolumn...")
                if 'TIMESTAMP' in merged_df.columns:
                    merged_df['TIMESTAMP'] = pd.to_datetime(merged_df['TIMESTAMP'], errors='coerce')
                
                # Próba konwersji pozostałych kolumn na typ numeryczny
                for col in merged_df.columns:
                    if col not in ['TIMESTAMP', 'group_id', 'source_file', 'interval']:
                        try:
                            # Próbujemy przekonwertować kolumnę na typ numeryczny
                            merged_df[col] = pd.to_numeric(merged_df[col])
                        except (ValueError, TypeError):
                            # Jeśli się nie uda (bo np. zawiera tekst), pozostawiamy ją bez zmian
                            pass
            # === KONIEC KLUCZOWEJ POPRAWKI ===
            
            # Krok 4: Uruchom PEŁEN potok korekt na kompletnych, surowych danych rocznych
            logging.info(f"Stosowanie korekt dla {len(merged_df)} wierszy z roku {year}...")
            corrected_df = apply_all_corrections(merged_df, config, context_name=f"Rok {year}")

            # Krok 5: Wywołaj dedykowaną funkcję zapisu do bazy danych SQLite
            if not corrected_df.empty:
                save_final_dataframe_to_csv(corrected_df, year, config, lock)

        except Exception as e:
            logging.error(f"Krytyczny błąd podczas finalnego przetwarzania roku {year}: {e}", exc_info=True)
            
def main():
    """
    Główna funkcja orkiestrująca w architekturze hybrydowej dla zapisu do CSV.
    """
    parser = argparse.ArgumentParser(description="Przetwarza i scala pliki danych do plików CSV.")
    ...
    group_config['output_dir'] = args.output_dir
    if args.input_dir:
        group_config['main_input_path'] = args.input_dir[0]
    ...
    with multiprocessing.Manager() as manager:
        ...
        # Ścieżka 1: Przetwarzanie plików binarnych
        if binary_files:
            logging.info(f"Przetwarzanie {len(binary_files)} plików binarnych (TOB1/TOA5) równolegle...")
        ...
        # Ścieżka 2: Przetwarzanie plików CSV
        if csv_files:
            ...
            for i in tqdm(batch_indices, desc="Partie plików CSV"):
                ...
                non_empty_dfs = [df for df in batch_dfs if not df.empty]
                ...
                batch_df = correct_and_report_chronology(batch_df, f"Partia {batch_num}")
                all_raw_results.append(batch_df)

    # Finalne przetwarzanie i zapis po zebraniu wszystkich surowych danych
    if all_raw_results:
        lock = multiprocessing.Manager().Lock()
        # Wywołujemy główną funkcję, teraz już bez argumentu use_sqlite
        process_and_save_yearly_data(all_raw_results, group_config, lock)
