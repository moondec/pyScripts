...
def validate_record_column(df, file_path, timestamp_col='TIMESTAMP'):
    ...
    if negative_indices or non_sequential_indices:
        ...
        print("#"*95 + "\n")
# === ZAAWANSOWANA WERSJA funkcji do czyszczenia i walidacji kolumny RECORD ===
# z ulepszonym komunikatem diagnostycznym

# def clean_and_validate_record_column(df: pd.DataFrame, file_path: str, timestamp_col='TIMESTAMP'):
    # """
    # Sprawdza spójność kolumny RECORD, raportuje anomalie i inteligentnie
    # obcina plik od miejsca wykrycia trwałego błędu.
    # """
    ...
    # # Krok 1: Usunięcie wierszy z RECORD = 0
    # initial_rows = len(df_cleaned)
    # # df_cleaned = df_cleaned[df_cleaned['RECORD'] != 0]
    # # rows_removed = initial_rows - len(df_cleaned)
    # # if rows_removed > 0:
        # # print(f"  Info (Walidacja RECORD): Usunięto {rows_removed} wierszy, gdzie RECORD = 0 w pliku {os.path.basename(file_path)}.")
    
    # # Upewnij się, że kolumna jest numeryczna po filtrowaniu
    # records = pd.to_numeric(df_cleaned['RECORD'], errors='coerce')
    # records.dropna(inplace=True)
    
    # # Użyjemy indeksu poprawnych rekordów do dalszych operacji
    # df_cleaned = df_cleaned.loc[records.index]
    # if df_cleaned.empty:
        # return df_cleaned

    # # Sprawdzenie 1: Wartości ujemne
    # negative_indices = records[records < 0].index.tolist()
    # if negative_indices:
         # # print(f"  Ostrzeżenie (Walidacja RECORD): Wykryto wartości ujemne w wierszach (indeksy): {negative_indices} w pliku {os.path.basename(file_path)}. Wiersze te i następne zostaną usunięte.")
         # print(f"  Ostrzeżenie (Walidacja RECORD): Wykryto wartości ujemne w wierszach (indeksy): {negative_indices} w pliku {file_path}. Wiersze te i następne zostaną usunięte.")
         # return df_cleaned.loc[:negative_indices[0]-1]

    # # Sprawdzenie 2: Błędy w sekwencji
    # diffs = records.diff()
    
    # # Znajdź wszystkie indeksy, gdzie krok nie jest poprawny
    # # Warunek prawidłowy: przyrost o 1 LUB duży spadek (reset licznika)
    # incorrect_steps = diffs[(diffs != 1) & (diffs >= -1000)]

    # if not incorrect_steps.empty:
        # # Przetwarzaj tylko jeśli istnieją jakiekolwiek błędy sekwencji
        # CONSECUTIVE_ERROR_TOLERANCE = 30
        # consecutive_errors = 0
        # first_error_index_in_sequence = None

        # for index, diff_value in incorrect_steps.items():
            # if first_error_index_in_sequence is None:
                # first_error_index_in_sequence = index
            
            # # Sprawdzamy, czy następny błąd jest bezpośrednio po poprzednim
            # if index == first_error_index_in_sequence + consecutive_errors:
                # consecutive_errors += 1
            # else: # Sekwencja błędów została przerwana
                # consecutive_errors = 1
                # first_error_index_in_sequence = index

            # if consecutive_errors >= CONSECUTIVE_ERROR_TOLERANCE:
                # # --- ZMODYFIKOWANY KOMUNIKAT ---
                # last_good_index_pos = df_cleaned.index.get_loc(first_error_index_in_sequence) - 1
                # last_good_record_val = df_cleaned.iloc[last_good_index_pos]['RECORD'] if last_good_index_pos >= 0 else "Brak (błąd na początku pliku)"

                # print("\n" + "#"*30 + " KOREKTA DANYCH: Błąd sekwencji RECORD " + "#"*30)
                # # print(f"  -> W pliku: {os.path.basename(file_path)}")
                # print(f"  -> W pliku: {file_path}")
                # print(f"  -> Ostatni poprawny numer rekordu: {last_good_record_val}")
                # print(f"  -> Wykryto {CONSECUTIVE_ERROR_TOLERANCE} lub więcej błędów z rzędu, zaczynając od wiersza o indeksie: {first_error_index_in_sequence}.")
                # print(f"  -> ODRZUCAM ten wiersz i wszystkie kolejne w tym pliku.")
                # print("#"*95 + "\n")
                
                # # Obetnij DataFrame w miejscu pierwszego błędu z serii
                # return df_cleaned.loc[:first_error_index_in_sequence-1]
...
# def main(input_dirs_arg, output_dir_arg, file_id_filter_arg, overwrite_arg):
    ...
    # for input_dir in input_dirs_arg:
        ...
        # for root, _, files in os.walk(input_dir):
            # for filename in files:
                # if filename.startswith('.') or not any(sid in filename for sid in source_ids_to_process): continue
    ...
    # for file_path in all_file_paths:
        ...
        # except Exception as e: print(f"Ostrzeżenie: Nie można wczytać pliku {os.path.basename(file_path)}. Błąd: {e}")
...
def main(input_dirs_arg, output_dir_arg, file_id_filter_arg, overwrite_arg):
        ...
        for root, _, files in os.walk(input_dir):
            for filename in files:
                if filename.startswith('.') or not any(sid in filename for sid in source_ids_to_process):
                    continue
                all_file_paths_to_check.append(os.path.abspath(os.path.join(root, filename)))
    
    all_file_paths_to_check.sort() 
    
    # Krok 2: Pętla przetwarzająca z logiką selektywnego cache
    print("\nKrok 2: Przetwarzanie plików z selektywnym cache...")
    list_of_dfs = []
    data_by_year = defaultdict(list)
    for file_path in all_file_paths_to_check:
        try:
            ...
            # Jeśli plik nie został pominięty, przetwarzaj go
            print(f"  -> Przetwarzam: {os.path.basename(file_path)}")
            # df, _ = read_and_process_one_file(file_path, effective_force_interval, file_id_filter_arg)
            df, _ = read_and_process_one_file(
                file_path,
                file_mtime, # <--- NOWO PRZEKAZYWANY ARGUMENT
                effective_force_interval,
                source_id_for_filename
            )
            if not df.empty:
                list_of_dfs.append(df)
                # Aktualizuj informacje dla cache
                file_mtime, file_size = os.path.getmtime(file_path), os.path.getsize(file_path)
                current_run_cache[file_path] = {'mtime': file_mtime, 'size': file_size}

        except Exception as e:
            print(f"  BŁĄD KRYTYCZNY podczas przetwarzania pliku {os.path.basename(file_path)}: {e}")

    if not list_of_dfs: print("Zakończono. Brak nowych lub zmienionych plików do przetworzenia."); return
    
    # Krok 3: Globalna agregacja i zapis (logika bez zmian)
    print(f"\nKrok 3: Finalna agregacja {len(list_of_dfs)} porcji danych i zapis...")
    master_df = pd.concat(list_of_dfs, ignore_index=True)
    del list_of_dfs
    
    # Logika deduplikacji i zapisu...
    initial_rows = len(master_df)
    cols_to_check = [col for col in master_df.columns if col not in ['source_file']]
    if 'TIMESTAMP' in cols_to_check:
    ...
    if len(final_df) < initial_rows:
        print(f"  Info (Deduplikacja): Scalono {initial_rows} wierszy do {len(final_df)} unikalnych rekordów.")
    ...
    for year, year_group in final_df.groupby(final_df['TIMESTAMP'].dt.year):
        ...
        print(f"  -> Zapisywanie pliku: {output_filename} (zawiera {len(year_group)} wierszy)")
        try:
            # W tej architekturze zawsze nadpisujemy pliki roczne
            year_group.to_csv(output_filepath, index=False)
        except Exception as e:
            print(f"  Krytyczny błąd podczas zapisu do pliku {output_filepath}: {e}")

    # Krok 4: Zapisz zaktualizowany cache
    processed_files_cache.update(current_run_cache)
    save_cache(processed_files_cache, CACHE_FILE_PATH)
    print(f"\nZakończono pomyślnie. Zaktualizowano {len(current_run_cache)} plików w cache.")
