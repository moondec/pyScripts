...
NAZWY_REMAPOWANE = {
    'PPFD_IN_1_1_1': 'PPFD_1_2_1',
    'PPFD_BC_IN_1_1_1': 'PPFD_BC_IN_1_1_1',
    'PPFD_BC_IN_1_1_1': 'PAR_up_1',
    'PPFD_BC_IN_1_1_2': 'PAR_up_2',
    'PPFD_BC_IN_1_1_2': 'PPFD_BC_IN_1_1_2',
    'PPFD_OUT_1_1_1': 'PPFDr_1_2_1',
    'PPFD_IN_1_1_3': 'PPFD_215up_Avg',
    'PPFD_DIF_1_1_2': 'PPFDd_Avg',
    'PPFD_IN_1_1_2': 'PPFD_1_1_1',
    'SW_OUT_1_1_1': 'SWout_1_2_1',
    'SW_IN_1_1_1': 'SWin_1_2_1',
    'PPFD_DIF_1_1_1': 'PPFDd_1_1_1'
}

def apply_column_mapping(df: pd.DataFrame, config: str) -> pd.DataFrame:
    """
...
def wczytaj_i_polacz_dane_csv(pliki_grupy: list) -> pd.DataFrame:
    ...
    # ZMIANA: Odwrócony słownik remapujący dla łatwiejszego wyszukiwania
    remap_odwrocony = {v: k for k, v in NAZWY_REMAPOWANE.items()}
    ...
    for sciezka in sorted(pliki_grupy):
        try:
            ...
            kolumny_do_wczytania = ['TIMESTAMP']
            # Szukaj NOWYCH nazw, które odpowiadają STARYM interesującym nazwom
            for nowa_nazwa, stara_nazwa in NAZWY_REMAPOWANE.items():
                if stara_nazwa in INTERESUJACE_CZLONY:
                    # Znajdź oryginalny nagłówek, który po wyczyszczeniu pasuje do nowej nazwy
                    for oryg, wyczyszczony in mapowanie_naglowkow.items():
                        if wyczyszczony == nowa_nazwa:
                            kolumny_do_wczytania.append(oryg)
            ...
            if len(kolumny_do_wczytania) > 1:
                ...
                # Zmień nazwy na czyste, NOWE nazwy
                df.rename(columns=mapowanie_naglowkow, inplace=True)
                # Zmień nazwy z NOWYCH na STARE (kanoniczne)
                df.rename(columns=NAZWY_REMAPOWANE, inplace=True)
                lista_df.append(df)
...
def wczytaj_i_polacz_dane_mat(wpisy_grupy: list) -> pd.DataFrame:
    ...
    if kolumna_korekty in df_laczny.columns:
        ...
        wspolczynnik = 3397.547 # 0.00029433
        
        maska = df_laczny.index > data_korekty
        df_laczny.loc[maska, kolumna_korekty] = df_laczny.loc[maska, kolumna_korekty] * wspolczynnik
        logging.info(f"Zastosowano korektę do kolumny '{kolumna_korekty}' dla danych MAT po dacie {data_korekty}.")
    kolumna_korekty = "PPFD_BC_IN_1_1_2"
    if kolumna_korekty in df_laczny.columns:
        data_korekty = pd.to_datetime("2018-11-16 11:09:00")
        wspolczynnik = 3288.716 # 0.00029433
...
# ZMIANA: NOWA, INTELIGENTNA FUNKCJA MAPUJĄCA
def mapuj_grupy(grupy_csv: dict, grupy_mat: dict) -> dict:
    zmapowane_grupy = {}
    mat_dopasowane = set()

    # Iteruj po grupach CSV i znajdź dla nich odpowiedniki w MAT
    for csv_nazwa, csv_pliki in grupy_csv.items():
        znaleziono_dopasowanie = False
        best_match = None
        
        for mat_nazwa in grupy_mat:
            # Sprawdź, czy krótsza nazwa grupy (zwykle MAT) zawiera się w dłuższej (zwykle CSV)
            # Ignoruje `_` i wielkość liter na potrzeby dopasowania
            if mat_nazwa.replace("_", "").lower() in csv_nazwa.replace("_", "").lower():
                best_match = mat_nazwa
                break # Znaleziono dobre dopasowanie

        if best_match:
            # Użyj nazwy z CSV jako kanonicznej, bo jest bardziej opisowa
            kanoniczna_nazwa = csv_nazwa
            zmapowane_grupy[kanoniczna_nazwa] = {'csv': csv_nazwa, 'mat': best_match}
            mat_dopasowane.add(best_match)
        else:
            # Grupa CSV bez odpowiednika w MAT
            zmapowane_grupy[csv_nazwa] = {'csv': csv_nazwa, 'mat': None}

    # Dodaj grupy MAT, które nie znalazły odpowiednika
    for mat_nazwa in grupy_mat:
...
if __name__ == '__main__':
    ...
    # sciezka_danych_csv = 'E:\\test_split\\TU'
    # sciezka_danych_mat = 'D:\\sites\\TU\\met-data_TU'
    # katalog_wyjsciowy = 'E:\\pdfy\\Porownanie_CSV_vs_MATLAB\\TU'
    ...
    # sciezka_danych_csv = 'E:\\test_split\\SA'
    # sciezka_danych_mat = 'D:\\sites\\SA\\met-data_SA'
    # katalog_wyjsciowy = 'E:\\pdfy\\Porownanie_CSV_vs_MATLAB\\SA'
